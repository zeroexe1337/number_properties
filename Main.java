...
public class Main {
                            ...
                            if (number == 0) {
                                ...
                                while (count < target) {
                                    ...
                                    for (String s : stringArray) {
                                        ...
                                        if (!hasMinus(s.toUpperCase(), number)) {
                                            addToList = false;
                                            break;
    ...
    static boolean checkProperty(String property, long num) {
        return switch (property) {
            ...
            case "JUMPING" -> checkJumping(num);
            case "PALINDROMIC" -> checkPalindromic(num);
            case "SPY" -> checkSpy(num);
            case "SQUARE" -> checkSquare(num);
            case "SUNNY" -> checkSunny(num);
            case "HAPPY" -> checkHappy(num);
            case "SAD" -> !checkHappy(num);
            default -> false;
        };
    }
